...
bool transform(arm_lib::Transform::Request &req, arm_lib::Transform::Response &res)
{
    ...
    // for (int i=0; i<3; i++) {
    //     ROS_INFO("Request Vectors: [%f]", req.input.vectors[i]);
    //     ROS_INFO("Request Angles: [%f]", req.input.angles[i]);
    //     ROS_INFO("Request d: %f", d);
    //     //res.output.transformed_vector[i] = req.input.vectors[i];
    //     //ROS_INFO("SENDING BACK REPONSE: [%f]", res.output.transformed_vector[i]);
    // }
...
std::vector<double> rot_trans(std::vector<double>vectors, std::vector<double>angles, double d)
    {   
        double x = vectors[0]; double y = vectors[1]; double z = vectors[2];
        double roll = M_PI * angles[0] / 180; double pitch = M_PI * angles[1] / 180; double yaw = M_PI * angles[2] / 180;
